...
/** This class creates a Controller for the ContactReport form. */
public class ContactReportController implements Initializable {
    ...
    public ComboBox<String> contactSelectorBox;
    /** This label displays the total number of appointments based on contactSelectorBox value. */
    public Label totalAppointments;
    public Button exitButton;
    /** This list holds appointments for TableView display based on selected Contact. */
    public ObservableList<Appointment> reportDisplay = FXCollections.observableArrayList();

    /** This is the override initialize method that executes when the ContactReport form is initialized.
     * The method populates the contactSelectorBox with all contacts using a helper method, and sets the exitButton action event to direct back to the Main form.
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        contactSelectorBox.setItems(Helper.getAllContacts());
        exitButton.setOnAction(actionEvent -> Helper.direct(ContactReportController.class, "/scheduling/schedulingapplication/View/Main.fxml", 1150, 665, "Scheduling Application", actionEvent));
    }

    /** This method implements the action event for the contactSelectorBox.
     * The method first clears the reportDisplay list of any previous appointments. It then adds relevant appointments to the reportDisplay list based on contactSelectorBox value. The method then updates the totalAppointments label with the list's size and populates the appointmentsTable TableView with the sorted reportDisplay list.
     * @param actionEvent
     */
    public void contactSelector(ActionEvent actionEvent) {
        /* Clear reportDisplay list */
        reportDisplay.clear();
        /* Add relevant appointments to reportDisplay list based on contactSelectorBox value */
        String boxValue = contactSelectorBox.getValue();
        for (Appointment apt : allAppointments) {
            if (apt.getContact().equals(boxValue)) {
